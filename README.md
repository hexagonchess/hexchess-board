...
`<hexchess-board>` now ships with pointers to a small, pre-generated sound pack (`move`, `capture`, `check`, `checkmate`, `victory`, `defeat`, `draw`). The default cue URLs are hosted on GitHub Pages and fetched at runtime, so the npm bundle stays lean. The files are preloaded as soon as the element is connected and replay instantly when moves resolve, check is declared, or the game finishes. Browsers still require a user gesture before audio playback, so either let players click/tap the board once or call `board.prepareAudio()` inside your own button handler.
...
- Default cue URLs point at `https://hexagonchess.github.io/hexchess-board/assets/audio/*.wav`. If you need to run offline or use your own branding, copy those files to your infra/CDN and point `soundPack` to the new URLs.
